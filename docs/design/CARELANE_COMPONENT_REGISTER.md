# Carelane — component & design-system register
...
**Evidence:** code inspection of `client/src` (App.tsx, `components/care/*`, `components/ui/*`, `components/design/*`, `components/navigation/*`, `lib/*`). Runtime confirmed the SPA is the live surface (see [`../audits/CARELANE_CURRENT_STATE_ASSESSMENT.md`](../audits/CARELANE_CURRENT_STATE_ASSESSMENT.md)).
...
| Buttons | `Button` · `ui/button.tsx`; `PrimaryActionButton` · `CareDesignPrimitives.tsx` | Base + Carelane primary |
...
New shared components must be added to this register with a status, and PRs should not introduce a second implementation of an Approved primitive (see [`../engineering/CARELANE_ENGINEERING_STANDARDS.md`](../engineering/CARELANE_ENGINEERING_STANDARDS.md) → component governance). A lint/guardrail that flags imports from Forbidden/Deprecated files is recommended (roadmap FE workstream).
