# CareOn Runbook Index

**Purpose:** single entry point for release-operations and production-readiness evidence.

## Indexed documents

- [Secrets Inventory](./SECRETS_INVENTORY.md)
- [Backup / Restore Drill](./BACKUP_RESTORE_DRILL.md)
- [Observability Checklist](./OBSERVABILITY_CHECKLIST.md)
- [Rollback Playbook](./ROLLBACK_PLAYBOOK.md)
- [Release Sign-off v1](../releases/RELEASE_SIGNOFF_V1.md)
- [Closed Pilot Release Packet v1](../releases/CLOSED_PILOT_RELEASE_PACKET_V1.md)

## Use

- Use this index when preparing a controlled pilot or production promotion.
- Use the individual documents to record evidence, owner assignments, and validation outcomes.
- Do not treat template documents as completed evidence until the checklist or evidence sections are filled with real observations.

