# Carelane — Phase 1 implementation plan (governance, verification & guardrails)
...
**Companions:** [`CARELANE_STANDARDIZATION_ROADMAP.md`](CARELANE_STANDARDIZATION_ROADMAP.md), [`../engineering/CARELANE_ENGINEERING_STANDARDS.md`](../engineering/CARELANE_ENGINEERING_STANDARDS.md), [`../design/CARELANE_COMPONENT_REGISTER.md`](../design/CARELANE_COMPONENT_REGISTER.md), [`../CARELANE_BACKEND_QUALITY_AUDIT_2026Q2.md`](../CARELANE_BACKEND_QUALITY_AUDIT_2026Q2.md).
...
| **Product decision needed?** | No |
| **Exit criteria** | Test green on `main`; covers (a) `reverse()`/`resolve()` round-trip for every named route, (b) importability of all listed cross-module symbols, (c) `carelane_views` attribute presence for every URL handler. **Designed to guard the later `views/legacy.py` + thin `__init__` + explicit re-export transition** |
...
| **Exact files** | `docs/design/CARELANE_COMPONENT_REGISTER.md` (exists); reference it from `.github/PULL_REQUEST_TEMPLATE.md` and `docs/engineering/CARELANE_ENGINEERING_STANDARDS.md`; optional `tests/test_component_register.py` asserting the file exists and parses (status labels present) |
...
| **Exact files** | `scripts/check_carelane_design_tokens.py` (exists; `npm run check:carelane-design`); add a **diff-scoped** CI step in `.github/workflows/ui-verification.yml` that runs it on changed `client/src` files |
...
| **Exact files** | `.github/PULL_REQUEST_TEMPLATE.md` (DoR checklist section) + `docs/engineering/CARELANE_ENGINEERING_STANDARDS.md` §1 (source) |
...
| **Exact files** | `.github/PULL_REQUEST_TEMPLATE.md` (DoD checklist) + `docs/engineering/CARELANE_ENGINEERING_STANDARDS.md` §2 |
| **Change type** | Additive (process doc + template checklist) |
| **Risk** | Very low |
