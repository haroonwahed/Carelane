# Carelane Structural Migration Plan
...
Replace the remaining legacy contract-platform runtime symbols and data model names with Carelane-native terminology without breaking tenant isolation, URLs, admin flows, or production data.
...
- Updated decision record in `docs/CARELANE_LEGACY_REMOVAL_MATRIX.md` showing these module families are already removed from live runtime.
- Cleanup of any active-code residue that still presents those modules as current Carelane product scope.
...
- Replace `carelane:` URL naming where still misleading.
...
4. Rename `Contract` to the chosen Carelane case model.
