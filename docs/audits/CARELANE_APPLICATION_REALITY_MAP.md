...
Volledige route-inventaris staat in `CARELANE_PRODUCT_UX_AUDIT.md` §A. Kernsamenvatting:
...
De backend dwingt deze volgorde af en staat geen overslaan toe behalve via expliciete, geauditeerde transities (`_ALLOWED_TRANSITIONS`, `evaluate_transition`). Volledige end-to-end-status per stap: zie `CARELANE_END_TO_END_FLOW_AUDIT.md`.
...
| **Legacy (parallel oppervlak)** | Server-rendered HTML-transitie-endpoints in `views.py` zijn default actief (`CARELANE_PILOT_SPA_ONLY=False`, `settings.py:317`) |
...
- **Feature flags:** `CARELANE_PILOT_SPA_ONLY`, `CARELANE_PILOT_UI`, `FEATURE_FREEZE_ACTIVE` (default True), `SSO_ENABLED`, `PILOT_AUTO_BOOTSTRAP`, `PILOT_FORCE_RESET`.
