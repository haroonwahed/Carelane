...
**Progress note (2026-06-17):** **Redesign wave (Q2)** is the de-facto active workstream and now leads scheduling: `--care-*` semantic design-token migration across care pages, shared **`CareSlaCountdown`** primitive adopted on Intake / Placement / Aanbiederreactie / Regiekamer, **Matching detail** restructured into a fixed-slot grid, interactive **Casus-detail** (owner/priority/elapsed/activity + guided completion with backend validation), and **Regiekamer** redesigned as an SLA action-list (dead legacy layout stripped). This is consolidation/UX hardening inside the **Infrastructure Maturity Phase**, not net-new scope. **Backend health:** `contracts/views.py` (~8.2k lines, ~320 defs) flagged as a god-module; decomposition into a `contracts/views/` package mirroring `contracts/api/` is specified in [`docs/adr/ADR-CONTRACTS-VIEWS-DECOMPOSITION.md`](./adr/ADR-CONTRACTS-VIEWS-DECOMPOSITION.md) (Proposed) — incremental, behaviour-preserving, green-at-every-step. Earlier provider-chain rows (below) remain valid law but are now interleaved with redesign + maintainability work.

**Progress note (2026-05-15):** Rows **6, 8, A** — Reacties **“Waarom deze aanvraag bij jullie ligt”** (`provider-review-why-us-block`); Playwright asserts why-us + reject → **Verwerkte aanvragen** + API `REJECTED`; `run_full_pilot_rehearsal.sh --with-playwright` rebuilds SPA, auto-**`--start-server`**; `run_golden_path_e2e` resets DB before browser smoke. Pilot gate: headless rehearsal **GO** + Playwright **12 passed / 1 skipped** — see current rollout evidence in [`RELEASE_EXECUTION_SHEET_2026-05-30.md`](./RELEASE_EXECUTION_SHEET_2026-05-30.md). Remaining: row **15** Could-band renames; optional accept-panel E2E when seeded card exposes capacity field.
