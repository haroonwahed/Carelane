# CareOn Design System & Core Experience Implementation Brief

## Role

Act as CareOn’s senior product designer, design-system architect, frontend architect, and implementation reviewer.

Your job is not to redesign isolated screens. Your job is to establish a complete, reusable, coded, documented, and enforceable CareOn design system, then apply it to the three highest-value operational pages.
...
- `CareOn_Product_Design_Constitution_v1_2.docx`
- `CareOn_Technical_Foundation_v1_2.docx`

Also inspect all current CareOn design documentation, shared components, layout primitives, theme tokens, route structure, state management, API clients, permissions, workflows, tests, and Storybook configuration if present.
...
Create a complete CareOn design-system foundation and apply it to these three pages:
...
CareOn is not a generic SaaS dashboard, KPI cockpit, CRM, or admin panel.
...
Translate the strongest patterns from mature healthcare platforms into CareOn:
...
## 1. `CAREON_DESIGN_SYSTEM_INVENTORY.md`
...
## 2. `CAREON_COMPONENT_DUPLICATION_REPORT.md`
...
## 3. `CAREON_UI_ACTION_BACKEND_MATRIX.md`
...
## 4. `CAREON_SCREEN_UNIFORMITY_MATRIX.md`
...
# Phase 4: Build CareOn Domain Components

Normalize existing CareOn components and add missing domain components.
...
Every CareOn component and state.
...
## `CAREON_DESIGN_SYSTEM_MIGRATION_PLAN.md`
...
## `CAREON_DESIGN_SYSTEM_GOVERNANCE.md`
