...
import { Language, t, formatNumber, formatDate } from "../lib/i18n";
...
function aggregateData(
  ...
  const buckets = new Map<string, { sales: number; count: number; firstDate: Date }>();
  const now = new Date();
  ...
  data.forEach(item => {
    ...
    const existing = buckets.get(key) || { sales: 0, count: 0, firstDate: date };
    buckets.set(key, {
  ...
  return sorted.map(([date, values], index) => ({
    date,
    sales: values.sales,
...
export function SalesChart({ data, delta, language, customFrom, customTo, aggregation = "daily", preset }: SalesChartProps) {
  ...
  const getXAxisInterval = () => {
    if (isHourly) return 3; // Show every 4th hour
    if (aggregation === "weekly" || aggregation === "monthly") return 0;
    if (chartData.length <= 7) return 0; // Show all days
    ...
    >
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="space-y-3">
          ...
          >
            {t(language, "chart.sales")} {isHourly && (
              <span 
          ...
          <div className="space-y-2">
            <div>
              ...
              >
                {formatNumber(totalSales, language)}
              </div>
