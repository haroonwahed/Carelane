import { TrendingUp, TrendingDown, Minus } from "lucide-react";
import { CHART_COLORS } from "../lib/chartColors";
import { Card } from "./ui/card";
import { Language, t, formatCurrency, formatDate } from "../lib/i18n";
import { DateRangePreset } from "./DateRangePicker";
import {
  LineChart,
  Line,
  XAxis,
  YAxis,
  CartesianGrid,
  Tooltip,
  ResponsiveContainer,
} from "recharts";

interface AverageBasketChartProps {
  data: Array<{
    date: string;
    avgBasket: number;
  }>;
  delta?: number;
  language: Language;
  customFrom?: Date;
  customTo?: Date;
  aggregation?: "hourly" | "daily" | "weekly" | "monthly";
  preset?: DateRangePreset;
}

// Helper to aggregate data
function aggregateData(
  data: Array<{ date: string; avgBasket: number }>,
  type: "hourly" | "daily" | "weekly" | "monthly",
  language: Language
) {
  if (type === "hourly" || type === "daily") {
    return data;
  }
  
  const buckets = new Map<string, { avgBasket: number; count: number; firstDate: Date }>();
  const now = new Date();
  const currentYear = now.getFullYear();
  
  data.forEach(item => {
    let date: Date;
    if (item.date.includes(':')) {
      return;
    } else if (item.date.includes('/')) {
      const parts = item.date.split('/');
      if (parts.length !== 2) return;
      
      const day = parseInt(parts[0], 10);
      const month = parseInt(parts[1], 10);
      
      if (isNaN(day) || isNaN(month) || day < 1 || day > 31 || month < 1 || month > 12) {
        return;
      }
      
      date = new Date(currentYear, month - 1, day);
      
      if (isNaN(date.getTime())) {
        return;
      }
      
      if (date > now) {
        date = new Date(currentYear - 1, month - 1, day);
        if (isNaN(date.getTime())) {
          return;
        }
      }
    } else {
      return;
    }
    
    let key: string;
    
    try {
      if (type === "weekly") {
        const dayOfWeek = date.getDay();
        const diff = dayOfWeek === 0 ? -6 : 1 - dayOfWeek;
        const weekStart = new Date(date);
        weekStart.setDate(date.getDate() + diff);
        key = formatDate(weekStart, language, { day: "numeric", month: "short" });
      } else {
        key = formatDate(date, language, { month: "short", year: "numeric" });
      }
    } catch (e) {
      return;
    }
    
    const existing = buckets.get(key) || { avgBasket: 0, count: 0, firstDate: date };
    buckets.set(key, {
      avgBasket: existing.avgBasket + item.avgBasket,
      count: existing.count + 1,
      firstDate: existing.firstDate,
    });
  });
  
  const sorted = Array.from(buckets.entries()).sort((a, b) => 
    a[1].firstDate.getTime() - b[1].firstDate.getTime()
  );
  
  return sorted.map(([date, values]) => ({
    date,
    avgBasket: values.avgBasket / values.count,
    _id: `${date}-${values.firstDate.getTime()}`, // Unique key
  }));
}

export function AverageBasketChart({ data, delta, language, customFrom, customTo, aggregation = "daily", preset }: AverageBasketChartProps) {
  
  // Check if we're in hourly mode
  const isHourly = aggregation === "hourly";
  
  // Aggregate data
  const aggregatedData = aggregateData(data, aggregation, language);
  
  // Add unique keys to data
  const chartData = aggregatedData.map((item, index) => ({
    ...item,
    _chartKey: `${item.date}-${index}`,
  }));
  
  // Determine x-axis interval based on data length
  const getXAxisInterval = () => {
    if (isHourly) return 3;
    if (aggregation === "weekly" || aggregation === "monthly") return 0;
    if (chartData.length <= 7) return 0;
    if (chartData.length <= 31) return Math.floor(chartData.length / 7);
    return "preserveStartEnd";
  };

  // Calculate average basket across all periods
  const avgBasket = data.reduce((sum, d) => sum + d.avgBasket, 0) / data.length;

  const formatCurr = (value: number) => {
    return formatCurrency(value, language).replace(/,00/g, '').replace(/\.00/g, '');
  };

  const getDeltaDisplay = (value: number) => {
    const isPositive = value > 0;
    const isNeutral = Math.abs(value) < 0.5;
    
    let colorClass = "";
    let DeltaIcon = Minus;
    
    if (!isNeutral) {
      if (isPositive) {
        colorClass = "text-green-base text-emerald-600";
        DeltaIcon = TrendingUp;
      } else {
        colorClass = "text-red-base text-red-600";
        DeltaIcon = TrendingDown;
      }
    } else {
      colorClass = "text-neutral-500";
    }
    
    const displayValue = isNeutral ? "0%" : `${isPositive ? "+" : ""}${value.toFixed(1)}%`;
    
    return (
      <div className={`inline-flex items-center gap-1.5 ${colorClass}`}>
        <DeltaIcon className="w-3.5 h-3.5" />
        <span className="text-xs" style={{ fontWeight: 500, fontSize: "12px" }}>
          {displayValue}
        </span>
      </div>
    );
  };
  
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      const currentValue = payload[0].value;
      
      return (
        <div 
          className="rounded-xl border bg-popover p-3 min-w-[180px]"
          style={{
            boxShadow: CHART_COLORS.cardGlow
          }}
        >
          <p className="text-muted-foreground mb-3" style={{ fontSize: "12px", fontWeight: 600 }}>
            {label}
          </p>
          <div className="space-y-1">
            <p className="text-muted-foreground" style={{ fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.05em" }}>
              {t(language, "chart.avgBasket")}
            </p>
            <p className="text-foreground" style={{ fontSize: "14px", fontWeight: 600 }}>
              {formatCurr(currentValue)}
            </p>
          </div>
        </div>
      );
    }
    return null;
  };

  return (
    <Card 
      className="rounded-2xl border p-6 bg-card shadow-sm"
      style={{
        boxShadow: CHART_COLORS.cardGlow
      }}
    >
      {/* Header with inline metrics */}
      <div className="flex items-start justify-between mb-6">
        <div className="space-y-3">
          <h3 
            className="text-foreground"
            style={{ fontSize: "18px", fontWeight: 600 }}
          >
            {t(language, "chart.avgBasket")} {isHourly && (
              <span 
                className="text-muted-foreground ml-2"
                style={{ fontSize: "14px", fontWeight: 400 }}
              >
                (24h)
              </span>
            )}
          </h3>
          
          <div className="space-y-2">
            <div>
              <div 
                className="text-foreground"
                style={{ fontSize: "24px", fontWeight: 700, letterSpacing: "-0.02em" }}
              >
                {formatCurr(avgBasket)}
              </div>
              <p className="text-muted-foreground text-xs">
                {t(language, "chart.average")}
              </p>
              {delta !== undefined && getDeltaDisplay(delta)}
            </div>
          </div>
        </div>
      </div>
      
      <div style={{ width: '100%', height: 300 }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData}>
            <CartesianGrid 
              strokeDasharray="0" 
              stroke={CHART_COLORS.grid}
              strokeWidth={1}
            />
            <XAxis 
              dataKey="date" 
              stroke={CHART_COLORS.axis}
              tick={{ fontSize: 13, fill: CHART_COLORS.axis }}
              tickLine={{ stroke: CHART_COLORS.grid }}
              axisLine={{ stroke: CHART_COLORS.grid }}
              interval={getXAxisInterval()}
            />
            <YAxis 
              stroke={CHART_COLORS.axis}
              tick={{ fontSize: 13, fill: CHART_COLORS.axis }}
              tickLine={{ stroke: CHART_COLORS.grid }}
              axisLine={{ stroke: CHART_COLORS.grid }}
              tickFormatter={(value) => formatCurr(value)}
            />
            <Tooltip content={<CustomTooltip />} />
            
            {/* Current period line */}
            <Line
              type="monotone"
              dataKey="avgBasket"
              stroke={CHART_COLORS.line}
              strokeWidth={2}
              dot={false}
              name={t(language, "chart.avgBasket")}
              isAnimationActive={false}
            />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </Card>
  );
}